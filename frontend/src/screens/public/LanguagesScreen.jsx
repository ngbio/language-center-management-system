...
export default function LanguagesScreen() {
  ...
  return <section className="languages-page">
    <div className="public-container">
      ...
      {loading ? <div className="language-card-grid">{[1, 2, 3].map((item) => <div className="language-card language-skeleton" key={item} />)}</div> : <div className="language-card-grid">{languages.map((language) => <Link className="language-card" to={`/ngon-ngu/${language.id}`} key={language.id}><span>{language.languageCode}</span><h2>{language.languageName}</h2><p>{language.description || "Khám phá các chương trình đào tạo và lộ trình phù hợp."}</p><strong>Xem chi tiết →</strong></Link>)}</div>}
