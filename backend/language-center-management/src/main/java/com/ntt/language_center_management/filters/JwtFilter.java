...
public class JwtFilter extends OncePerRequestFilter {
  ...
  protected void doFilterInternal(
    ...
    if (!header.startsWith(BEARER_PREFIX)) {
      sendJsonError(
          response, HttpServletResponse.SC_UNAUTHORIZED, "Authorization header không hợp lệ");
      return;
    }

    String token = header.substring(BEARER_PREFIX.length()).trim();
    if (token.isEmpty()) {
      sendJsonError(response, HttpServletResponse.SC_UNAUTHORIZED, "Token không được để trống");
      return;
    }
